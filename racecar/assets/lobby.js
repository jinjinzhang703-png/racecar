...
//  网络消息处理
// ============================================================
let netHandlersInstalled = false;
function setupNet(){
  if(netHandlersInstalled) return;
  netHandlersInstalled = true;
  Net.onPeer((gid, ev)=>{
    if(!MP.active) return;
    if(!MP.isHost){
...
function handleHostMessage(fromId, m){
  switch(m.t){
    case 'join': {
      if(!slotPool.length){
        Net.sendTo(fromId, { t:'error', msg:'房间已满' });
        break;
      }
      ...
      players.push({
        ...
        ready:false, slot: slotPool.shift(),
      });
      broadcastLobby();
      break;
    }

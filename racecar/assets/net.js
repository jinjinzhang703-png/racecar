...
const Net = (function(){
  ...
  // 等待与房主的 DataChannel 打通 (30s 超时)
  function waitOpen(timeoutMs = 30000){
  ...
  //  公共
  // ============================================================
  // 重置所有连接状态 (返回主菜单/离开大厅时调用)
  function reset(){
    // 关闭客人端连接
    if(pc){ try{ pc.close(); }catch(e){} pc = null; dc = null; }
    // 关闭房主端所有客人连接
    guests.forEach(g=>{ try{ g.pc.close(); }catch(e){} });
    guests.clear();
    // 关闭待接入连接并清除超时
    pendings.forEach(p=>{
  ...
  return {
    ...
    join, waitOpen,
    connectServer, serverAvailable, reset,
    send(obj){

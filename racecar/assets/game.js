...
function updatePlayer(dt){
  ...
  // 倒车模式下 ERS/DRS/tow 直接禁用 (这些是前进动力)
  if(c.inReverse){ ersBoost = 0; drsBoost = 0; tow = 0; }
  ...
  if(!isFinite(c.gear)) c.gear=1; // NaN 防护
...
function integrate3D(c, dt){
  // NaN/Infinity 止损: 姿态异常时直接重置回赛道, 防止永久污染
  if(!isFinite(c.y) || !isFinite(c.vy) || !isFinite(c.roll) || !isFinite(c.rollV) ||
     !isFinite(c.pitch3d) || !isFinite(c.pitchV) || !isFinite(c.pos.x) || !isFinite(c.pos.z)){
    c.y = 0; c.vy = 0; c.roll = 0; c.rollV = 0; c.pitch3d = 0; c.pitchV = 0;
    c.airborne = false; c.flipped = false;
    resetCarToTrack(c);
    return;
  }
  // ---- 离地飞行: 弹道 + 角动量守恒 ----
...
function applyCarLivery(c, cfg){
  if(!c || !c.mesh || !c.mesh.userData) return;
  ...
  if(c.mesh.userData.sidepodMat){
    ...
    if(c.mesh.userData.sidepodMat.map) c.mesh.userData.sidepodMat.map.dispose();
...
function startRace(config){
  ...
  // 清理旧车辆资源 (几何体/材质/纹理), 防止多局后 GPU 内存泄漏
  const oldCars = scene.children.filter(o=>o.userData&&o.userData.isCar);
  for(const o of oldCars){
    scene.remove(o);
    o.traverse(child=>{
...
function updateWarnings(){
  ...
  // 速度警告 (接近弯道时速度过高)
  if(ui.speedo){
    if(!isFinite(c.speed)){ ui.speedo.classList.remove('speedWarn'); return; }
...
function handlePitGameKey(code){
  ...
  if(pressed === expected){
    ...
    const panel = $('pitGamePanel');
    if(panel){
      if(pitGame._failTimeoutId) clearTimeout(pitGame._failTimeoutId);
      panel.classList.add('pit-fail');
      pitGame._failTimeoutId = setTimeout(()=>{ panel.classList.remove('pit-fail'); pitGame._failTimeoutId = null; }, 300);
    }
...
function completePitStop(penalized){
  pitGame.active = false;
  if(pitGame._failTimeoutId){ clearTimeout(pitGame._failTimeoutId); pitGame._failTimeoutId = null; }
  const el = $('pitGamePanel');
  ...
  player.pits++;
  player.tire = 100;
  player._tireProtect = 0.5; // 换胎后 0.5s 内不磨损, 防止测试/逻辑检查立即失败
  // 保留玩家选择的轮胎类型, 不硬编码为 'S'
...
function tryPit(){
  ...
  if(c.airborne || c.flipped){ flashMsg('NO PIT','车辆离地/翻车中'); return; }
  if(Math.abs(c.speed) > PIT_SPEED_LIMIT){ flashMsg('NO PIT','速度过高, 请减速进站'); return; }
...
function resetCarToTrack(c){
  if(!SAMPLES.length || !curve) return;
  const near=nearestSample(c.pos);
...
function playCrashSound(intensity){
  if(!audioInitialized || !audioCtx || !audioMaster) return;
...
function playCrackle(vol){
  if(!audioInitialized || !audioCtx || !windSrc || !windSrc.buffer) return;
...
function updateTireSound(slipAmount){
  ...
  const freq = 900+slipAmount*700;
  tireFilter.frequency.linearRampToValueAtTime(isFinite(freq) ? freq : 900, t+0.05);
}
...
function updateEngineSound(){
  ...
  const safeF = isFinite(f) ? f : 70;
  engOsc1.frequency.linearRampToValueAtTime(safeF, t+0.05);
  engOsc2.frequency.linearRampToValueAtTime(safeF*2.02, t+0.05); // 二次谐波失谐
  engSub.frequency.linearRampToValueAtTime(Math.max(30, safeF*0.5), t+0.05);
  // 滤波器: 随 RPM 和油门负载开放
  const filterF = 250+rn*1800+(input.acc?300:0);
  engFilter.frequency.linearRampToValueAtTime(isFinite(filterF) ? filterF : 400, t+0.05);
  // 音量: 怠速低沉, 随 RPM+油门增大
  const vol=Math.min(0.05+rn*0.11+(input.acc?0.07:0), 0.23);
  engGain.gain.linearRampToValueAtTime(isFinite(vol) ? vol : 0, t+0.05);
  // 风噪 ∝ 速度² (高速呼啸)
...
function updateRemoteCar(c){
  ...
  let a=c._netPrev;
  if(!a){
    // 第一帧: 从当前位置平滑过渡到网络位置, 避免瞬移
    c._netPrev = { p:[c.pos.x, c.pos.y, c.pos.z], h:c.heading, v:c.speed, t:b.t - 120 };
    a = c._netPrev;
  }
  if(b.t<=a.t){
    // 过期/重复消息: 直接采用最新状态
...
function loop(){
  ...
  if(race.phase==='racing'||race.phase==='grid'){
    ...
    // 换胎保护期: _tireProtect > 0 时不磨损
    if(player._tireProtect > 0){
      player._tireProtect -= dt;
    } else {
